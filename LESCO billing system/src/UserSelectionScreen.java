...
public class UserSelectionScreen extends JFrame {
    ...
    public UserSelectionScreen() {
        ...
        // Load separate icons for employee and customer
        ImageIcon employeeIcon = new ImageIcon("src/resources/employee.png");
        ImageIcon customerIcon = new ImageIcon("src/resources/customer.png");
        Image scaledImage1 = employeeIcon.getImage().getScaledInstance(30, 30, Image.SCALE_SMOOTH);
        ImageIcon scaledOriginalIcon1 = new ImageIcon(scaledImage1);

        Image scaledImage2 = customerIcon.getImage().getScaledInstance(30, 30, Image.SCALE_SMOOTH);
        ImageIcon scaledOriginalIcon2 = new ImageIcon(scaledImage2);



        // Create labels with background color
        RoundedLabel employeeLabel = new RoundedLabel(scaledOriginalIcon1,"   Employee", new Color(121, 87, 87), 56, 56);
        RoundedLabel customerLabel = new RoundedLabel(scaledOriginalIcon2,"   Customer", new Color(121, 87, 87), 56, 56);

        // Set different icons, font, and alignment for labels
        setupLabel(employeeLabel, employeeIcon);
        setupLabel(customerLabel, customerIcon);

        // Calculate positions for centering the labels
        employeeLabel.setSize(200, 50); // Fixed label size
        customerLabel.setSize(200, 50);

        int centerX = (getWidth() - employeeLabel.getWidth()) / 2; // Horizontal center
        int employeeLabelY = (getHeight() - employeeLabel.getHeight()) / 3; // One-third down from the top
        int customerLabelY = employeeLabelY + 70; // Below the employee label

        employeeLabel.setLocation(centerX, employeeLabelY);
        customerLabel.setLocation(centerX, customerLabelY);
